<x-app-layout>
    <div class="min-h-screen bg-[#f3f2ef] py-8">
        <div class="max-w-4xl mx-auto px-6">
            <div class="mb-8">
                <h1 class="text-3xl font-bold text-gray-900">Edit Job Post</h1>
                <p class="mt-2 text-gray-600">
                    Update your listing details and keep your post accurate.
                </p>
            </div>

            <div class="bg-amber-50 border border-amber-200 text-amber-800 rounded-2xl p-4 mb-6">
                Any changes to this job post will send it back for admin approval before it becomes visible again.
            </div>

            <div class="bg-white border border-gray-200 rounded-2xl p-6 shadow-sm">
                <form action="{{ route('employer.job-posts.update', $job) }}" method="POST" class="space-y-6">
                    @csrf
                    @method('PUT')
                    ...
                    <div>
                        ...
                        <textarea name="description" rows="7"
                                  class="w-full border border-gray-300 rounded-xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-blue-500">{{ old('description', $job->description) }}</textarea>
                        @error('description')
                            <p class="text-red-600 text-sm mt-2">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="flex gap-3 pt-2">
                        <button type="submit"
                                class="bg-blue-600 text-white px-6 py-3 rounded-full font-medium hover:bg-blue-700">
                            Update Job Post
                        </button>

                        <a href="{{ route('employer.job-posts.index') }}"
                           class="border border-gray-300 text-gray-700 px-6 py-3 rounded-full">
                            Cancel
                        </a>
                    </div>
