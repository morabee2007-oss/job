                    ...
                    <div class="bg-white border border-gray-200 rounded-2xl p-6 shadow-sm">
                        ...
                        <div class="grid md:grid-cols-2 gap-4 mt-4">
                            <div class="bg-gray-50 rounded-xl p-4 border border-gray-100">
                                ...
                                <p class="mt-1 font-semibold text-gray-900">
                                    {{ $job->approver?->name ?? 'Not approved yet' }}
                                </p>
                            </div>
                            ...
                            <div class="bg-gray-50 rounded-xl p-4 border border-gray-100">
                                ...
                                <p class="mt-1 font-semibold text-gray-900">
                                    {{ optional($job->approved_at)->format('Y-m-d H:i') ?? 'Not approved yet' }}
                                </p>
                            </div>
                        </div>
                        ...
                        <div class="mt-8 flex flex-wrap gap-3">
                            @if($job->approval_status !== 'approved')
                                <form action="{{ route('admin.job-posts.approve', $job) }}" method="POST">
                                    @csrf
                                    @method('PATCH')
                                    <button type="submit"
                                            class="inline-flex items-center justify-center h-11 px-5 rounded-full bg-green-600 text-white text-sm font-semibold hover:bg-green-700 transition">
                                        Approve
                                    </button>
                                </form>
                            @endif
                            ...
                            @if($job->approval_status !== 'rejected')
                                <form action="{{ route('admin.job-posts.reject', $job) }}" method="POST">
                                    @csrf
                                    @method('PATCH')
                                    <button type="submit"
                                            class="inline-flex items-center justify-center h-11 px-5 rounded-full bg-amber-500 text-white text-sm font-semibold hover:bg-amber-600 transition">
                                        Reject
                                    </button>
                                </form>
                            @endif
                ...
                <div class="lg:col-span-4">
                    <div class="bg-white border border-gray-200 rounded-2xl p-6 shadow-sm">
                        ...
                        <p class="mt-2 text-sm text-gray-600">
                            Review candidate applications for this job.
                        </p>
